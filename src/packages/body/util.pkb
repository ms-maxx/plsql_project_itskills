create or replace package body util is
  ...
  begin
    select sys.get_nbu(p_url => 'https://bank.gov.ua/NBUStatService/v1/statdirectory/exchange?valcode=' ||
                                p_valcode || '&date=' || v_date || '&json') as res
      into v_json
      from dual;
    return v_json;
  end get_needed_curr;

  function is_check_time(p_date in date default sysdate) return boolean is
  ...
  procedure add_employee_history(p_employee_id   in number,
                                 ...
                                 p_salary        in number,
                                 p_hire_date     in date,
                                 p_fire_reason   in varchar2 default null) is
  ...
  begin
    ...
    begin
      select 1 into v_count_job_id from jobs j where j.job_id = p_job_id;
    exception
      when no_data_found then
        raise_application_error(-20001,
                                'A non-existent job_id was entered.');
    ...
    exception
      when no_data_found then
        raise_application_error(-20002,
                                'A non-existent department_id was entered.');
      when too_many_rows then
        raise_application_error(-20005, 'More then one line found.');
    end;
    ...
    begin
      v_message := 'Employee: ' || p_first_name || ' ' || p_last_name ||
                   '. Job_id: ' || p_job_id || '. Department id: ' ||
                   p_department_id || '. successfully added to the system.';
  ...
  begin
    ...
    exception
      when no_data_found then
        raise_application_error(-20001,
                                'A non-existent employee_id was entered.');
      when too_many_rows then
        raise_application_error(-20005, 'More then one line found.');
    end;
    ...
    begin
      ...
      v_message := 'Employee: ' || v_info_employee.first_name || ' ' ||
                   v_info_employee.last_name || '. Job_id: ' ||
                   v_info_employee.job_id || '. Department id: ' ||
                   v_info_employee.department_id ||
  ...
  begin
    ...
    if p_first_name is null and p_last_name is null and p_email is null and
      ...
      raise_application_error(-20001,
                              'No attributes were passed for update');
    end if;
    ...
    dbms_output.put_line('The employees: ' || p_employee_id ||
                         ' attributes heve been successfully updated');
  ...
  procedure api_nbu_sync is
    ...
    c_err_param_is_empty constant pls_integer := -20111;
  begin
    log_utils.log_start(p_proc_name => c_proc_name);
    
    begin
      select t.value_text
      into v_list_currencies
      from sys_params t
      where t.param_name = c_param_name;
    
    exception
      when no_data_found then
        log_utils.log_error(p_proc_name => c_proc_name,
                            p_sqlerrm   => sqlerrm,
                            p_text => 'Parameter: '||c_param_name
                            ||' not found in SYS_PARAMS');
        raise_application_error (c_err_param_not_found, 
                                 'Parameter ' || c_param_name || ' not found in SYS_PARAMS');
      when too_many_rows then 
        log_utils.log_error(p_proc_name => c_proc_name,
                            p_sqlerrm   => sqlerrm,
                            p_text => 'More than one row found for parameter ' || c_param_name);
        raise_application_error (c_err_param_not_found, 
                                 'More than one row found for parameter ' || c_param_name);
      when others then
        log_utils.log_error(p_proc_name => c_proc_name,
                            p_sqlerrm   => sqlerrm);
        raise;
    ...
    if v_list_currencies is null then 
        log_utils.log_error(p_proc_name => c_proc_name,
                            ...
                            p_text => 'Parameter: '||c_param_name
                            ||' is empty');
        raise_application_error(c_err_param_is_empty,
                                'Parameter: '||c_param_name ||' is empty');
    end if;
  
    for cc in (
      select value(t) as curr
        from table(util_ddl.table_from_list(p_list_val => v_list_currencies))t
    ) 
    loop
      begin
        insert into cur_exchange
          (r030, txt, rate, cur, exchangedate)
        select jt.r030,
               jt.txt,
               jt.rate,
               jt.cur,
               to_date(jt.exchangedate, 'dd.mm.yyyy') as exchangedate
          from (
                select util.get_needed_curr(
                         p_valcode => cc.curr,
                         p_date    => sysdate
                       ) as json_value
                  from dual
          ...
          cross join json_table(
            ...
            '$[*]'
            columns (
              r030         number(6)         path '$.r030',
              txt          varchar2(200 char) path '$.txt',
              rate         number(20,10)      path '$.rate',
              cur          varchar2(20 char)  path '$.cc',
              exchangedate varchar2(20 char)  path '$.exchangedate'
            )
          ) jt;
      exception 
        when others then
           log_utils.log_error(p_proc_name => c_proc_name,
                               p_sqlerrm   => sqlerrm,
                               p_text      => 'Error while syncing currency ' || cc.curr);
           continue;
      end;                    
    end loop;
    
    log_utils.log_finish(p_proc_name => c_proc_name);
  exception 
     when others then
      log_utils.log_error(
        p_proc_name => c_proc_name,
        p_sqlerrm   => sqlerrm,
